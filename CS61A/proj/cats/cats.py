...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    tmp=[paragraphs[i] for i in range(len(paragraphs)) if select(paragraphs[i])]
    if k>len(tmp)-1:    
        return ''
    else:
        return tmp[k]
...
def about(subject):
    ...
    def select(paragraph):
        check_paragraph = split(lower(remove_punctuation(paragraph)))
        for word in subject:
            if word in check_paragraph:
                return True
        return False
    return select
    # END PROBLEM 2
...
def minimum_mewtations(typed, source, limit):
    ...
    # answer1 动态规划
    a = len(typed)  # typed 的长度
    b = len(source)  # source 的长度
    
    # 初始化动态规划表 dp，大小为 (a+1) x (b+1)
    dp = [[0] * (b+1) for _ in range(a+1)]
    
    # 初始化第一行和第一列
    for i in range(1, b+1):
        dp[0][i] = i  # 将空字符串转换为 source 的前 i 个字符需要 i 次插入操作
    for i in range(1, a+1):
        dp[i][0] = i  # 将 typed 的前 i 个字符转换为空字符串需要 i 次删除操作
    
    # 填充动态规划表
    for i in range(0, a):
        for j in range(0, b):
            if typed[i] == source[j]:  # 如果字符相同，不需要额外操作
                dp[i+1][j+1] = dp[i][j]
            else:  # 如果字符不同，取三种操作的最小值 + 1
                dp[i+1][j+1] = min(
                    dp[i][j] + 1,      # 替换操作
                    dp[i][j+1] + 1,    # 删除操作
                    dp[i+1][j] + 1     # 插入操作
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        ...
        fastest[min_player_index].append(get_word(match, word_index))
